...
def update_data_in_sql(item_id):
    ...
    cursor.execute("SELECT name, type, level, flags, rarity, icon, buy_price, buy_quantity, sell_price, sell_quantity FROM items WHERE id=?", (item_id,))
    row = cursor.fetchone()

    # If the data is not aligned, update it in the SQL file
    if not row or row != (name, item_type, level, flags, rarity, icon, buy_price, buy_quantity, sell_price, sell_quantity):
        cursor.execute(
...
def main():
    ...
    for item_id in item_ids:
        ...
        if count % 100 == 0:
            ...
            print(f"Processed {count} of {len(item_ids)} items in {format_time(elapsed_time)}...")
