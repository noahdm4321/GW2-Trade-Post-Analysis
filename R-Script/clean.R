...
query <- "SELECT [item_id], [buy_price], [buy_quantity], [sell_price], [sell_quantity] FROM [prices]"
...
# Add profit column
item_data$profit <- item_data$sell_price-1 - ceiling((item_data$buy_price+1) * 0.85)
# Add profit column for buy now and sell now
item_data$profit_now <- ceiling((item_data$buy_price) * 0.85) - item_data$sell_price

#################################################################################

# Create dataframe of investment oportunities
...
# Create dataframe of investment_now oportunities
